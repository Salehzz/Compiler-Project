...
class Main inherits IO {
   ...
   mylist : List;
   mylist1 : List;
   print_list(l : List , i1:Int) : Object
    {
       if(l.isNil()) then {out_string("\n");}
       else if(not(l.head()=i1)) then
       {
               out_int(l.head());
			   out_string(" ");
			   print_list(l.tail(),l.head());
        }
        else {print_list(l.tail(),l.head());}
        fi fi
   };
    i:Int;
    j:Int;
   ...
   main() : Object {
     ...
     (let i:Int<-0,n:Int in{
         ...
         while (i < n) loop {
          ...
          print_list(sort(mylist),sort(mylist).head()-1);
      }
   };
